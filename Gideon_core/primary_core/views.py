...
def gideon_speaks(output):
    ...
    with open('gideon.mp3', 'wb') as f:
        f.write(r.content)
    print("Gideon: ", output)
    playsound.playsound('gideon.mp3', True)
    os.remove('gideon.mp3')
    t.sleep(1)
...
def set_alarm(alarmtime, day="today", msg=""):
    ...
    try:
        ...
        try:
            ...
            t1.start()
            if len(msg) == 0:
                gideon_speaks("Time's up!")
            else:
                gideon_speaks("Reminder for " + msg)
            while True:
                k = 1
...
def wiki(input_gideon):
    if 'wikipedia' in input_gideon:
        ...
        try:
            ...
            gideon_speaks("Could not find anything about " + statement + " on Wikipedia")
...
def google_search(input_gideon=""):
    ...
    # process.start()
    # t.sleep(3)
    wiki("wikipedia " + query2)
    gideon_speaks("Showing results for " + query2 + " on Google")
    gs = 'https://www.google.com/search?q=' + query
    webbrowser.open(gs)
...
def auto_battery_check():
    while True:
        ...
        if battery.power_plugged and battery.percent == 100:
            ...
            playsound.playsound('ironman_backup_power.mp3')
            t.sleep(300)
            continue
        elif battery.percent < 20 and not battery.power_plugged:
            chrg_stat = "critically low"
...
# =============================================================================
        ...
        webbrowser.open(gs)
    elif ("a u m s" in input_gideon or "a m s" in input_gideon) and "open" in input_gideon:
        gs = 'https://aumsblr.amrita.edu/cas/login?service=https%3A%2F%2Faumsblr.amrita.edu%2Faums%2FJsp%2FCore_Common%2Findex.jsp%3Ftask%3Doff'
        gideon_speaks("Opening A U M S")
        webbrowser.open(gs)
        t.sleep(5)
        pa.click(953, 355)
...
def index(request):
    if request.method == 'POST':
        ...
        print(data['text'])
        if data['text'] == 'wake':
            # gideon_speaks("Hello, I am Gideon, your personal assistant")
            main()
            return HttpResponse('Hello')
        if data['text'] == "text wake":
            text = str(data["inp"])
            print(text)
